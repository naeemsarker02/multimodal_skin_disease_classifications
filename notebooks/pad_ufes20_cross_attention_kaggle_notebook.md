...
Structured for "Save & Run All (Commit)" from the start, same process as the Stage 1 baseline and late-fusion notebooks. Requires **three** Kaggle "Add Data" sources attached to the notebook before running - identical sources to the Stage 1 late-fusion notebook, since Stage 2 warm-starts from the same Stage 1 image/metadata checkpoints:

- `mahdavi1202/skin-cancer` (raw PAD-UFES-20 image mirror)
- `naeemsarkertracer/pad-ufes20-processed` (our processed metadata CSVs)
- `naeemsarkertracer/pad-ufes20-stage1-checkpoints` (published — https://www.kaggle.com/datasets/naeemsarkertracer/pad-ufes20-stage1-checkpoints), containing the Stage 1 checkpoints (`image_seed{0,1,2}_best.pt`, `metadata_seed{0,1,2}_best.pt`). Cross-attention fusion warm-starts from these; it cannot proceed without them.
...
## Cell 10 — `%%writefile /kaggle/working/src/models/train_cross_attention_fusion.py`
...
## Cell 11 — Sanity check (config load + Stage 1 checkpoint resolution + real image path resolution)
...
## Cell 12 — Full model/GPU/dependency check (cross-attention model, warm-start, one real batch)
...
## Cell 13 — Train: cross-attention fusion, seed 0
...
## Cell 14 — Train: cross-attention fusion, seed 1

```python
!python -m src.models.train_cross_attention_fusion --dataset PAD_UFES20 --seed 1
```

---

## Cell 15 — Train: cross-attention fusion, seed 2

```python
!python -m src.models.train_cross_attention_fusion --dataset PAD_UFES20 --seed 2
```
