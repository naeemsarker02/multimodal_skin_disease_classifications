...
**Why this notebook exists:** the local dev machine has ~7.4GB RAM and the 3-seed CPU run was repeatedly OOM-killed by the OS, even one seed at a time - this is not a code issue (a 1-epoch local smoke test passed cleanly first). Offloading to Kaggle's free GPU, same reason Step 3's 5-backbone comparison ran there.

**This is a VAL-ONLY experiment - `train_cross_attention_supcon.py` never reads `metadata_test.csv`.** L = CE_loss (class-weighted, identical weights to the headline) + lambda * SupCon_loss (Khosla et al. 2020, lambda=0.1 default), applied to the 320-d fused (post-cross-attention, pre-classifier) feature vector. Same warm-start, LR, batch size, epoch cap, and early-stopping settings as the locked headline `train_cross_attention_fusion.py` run - only the loss function changes. Writes to new `cross_attention_supcon_*` checkpoint/log names; the locked headline checkpoints (`cross_attention_seed{0,1,2}_best.pt`) are never opened for writing.

Structured for "Save & Run All (Commit)", same process as every prior notebook. Requires **three** Kaggle "Add Data" sources attached before running - identical sources to the headline cross-attention notebook, since this warm-starts from the same Stage 1 image/metadata checkpoints:

- `mahdavi1202/skin-cancer` (raw PAD-UFES-20 image mirror)
- `naeemsarkertracer/pad-ufes20-processed` (our processed metadata CSVs)
- `naeemsarkertracer/pad-ufes20-stage1-checkpoints` (Stage 1 `image_seed{0,1,2}_best.pt` / `metadata_seed{0,1,2}_best.pt`) - this run cannot proceed without them.

After "Save & Run All (Commit)" finishes, download the notebook's Output (Kaggle preserves everything under `/kaggle/working`) and copy back into this repo:
...
## Cell 9 — `%%writefile /kaggle/working/src/models/losses.py`

```python
%%writefile /kaggle/working/src/models/losses.py
...
## Cell 10 — `%%writefile /kaggle/working/src/models/backbones.py`
...
## Cell 11 — `%%writefile /kaggle/working/src/models/train.py`
...
## Cell 12 — `%%writefile /kaggle/working/src/models/train_cross_attention_supcon.py`

```python
%%writefile /kaggle/working/src/models/train_cross_attention_supcon.py
...
## Cell 13 — Sanity check (config load + Stage 1 checkpoint resolution + real image path resolution)
...
## Cell 14 — Full model/GPU/dependency check (cross-attention model, warm-start, SupCon loss, one real batch)
...
## Cell 15 — Train: cross-attention + SupCon, seed 0

```python
!python -m src.models.train_cross_attention_supcon --seed 0 --lambda-supcon 0.1
```

---

## Cell 16 — Train: cross-attention + SupCon, seed 1

```python
!python -m src.models.train_cross_attention_supcon --seed 1 --lambda-supcon 0.1
```

---

## Cell 17 — Train: cross-attention + SupCon, seed 2

```python
!python -m src.models.train_cross_attention_supcon --seed 2 --lambda-supcon 0.1
```

---

## Cell 18 — Combine all 3 seeds' val results (mirrors `main()`'s combined-results write)
