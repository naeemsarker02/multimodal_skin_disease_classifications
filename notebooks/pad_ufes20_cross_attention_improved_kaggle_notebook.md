# PAD-UFES-20 Kaggle Notebook — Score Improvement Experiments: Combined Best-Effort Cross-Attention Run
...
Structured for "Save & Run All (Commit)" from the start. Trains **3 new checkpoints** (`cross_attention_improved_seed{0,1,2}_best.pt`), one combined run stacking every legitimate training-strategy improvement on top of the existing Phase 7 Stage 2 cross-attention architecture: Focal Loss (gamma=2.0, existing class-weighted alpha), WeightedRandomSampler oversampling of rare classes, stronger augmentation (RandomRotation(30) + RandomAffine on top of the existing flip/color jitter), label smoothing (0.1), and cosine annealing LR (1e-5 -> 1e-7). This is the final reported number for this architecture/dataset - not a separate ablation track.

Requires the same **three** Kaggle "Add Data" sources as the Phase 7 Stage 2 notebook (no new upload needed):

- `mahdavi1202/skin-cancer` (raw PAD-UFES-20 image mirror)
- `naeemsarkertracer/pad-ufes20-processed` (our processed metadata CSVs)
- `naeemsarkertracer/pad-ufes20-stage1-checkpoints` (Stage 1 `image_seed{0,1,2}_best.pt` / `metadata_seed{0,1,2}_best.pt` - warm-start source)
...
## Cell 10 — `%%writefile /kaggle/working/src/models/train_cross_attention_improved.py`

```python
%%writefile /kaggle/working/src/models/train_cross_attention_improved.py
...
## Cell 11 — Sanity check (warm-start checkpoints resolve)
...
## Cell 12 — Train: cross_attention_improved, seed 0
...
## Cell 13 — Train: cross_attention_improved, seed 1

```python
!python -m src.models.train_cross_attention_improved --dataset PAD_UFES20 --seed 1
```

---

## Cell 14 — Train: cross_attention_improved, seed 2

```python
!python -m src.models.train_cross_attention_improved --dataset PAD_UFES20 --seed 2
```
