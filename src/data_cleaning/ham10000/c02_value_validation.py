...
Flags implausible values (out-of-range age, non-standard sex
...
from src.data_cleaning.config import HAM10000_INTERIM_DIR
...
def run(logger, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def flag(mask, column, reason):
        for idx in df[mask].index:
            flags.append(
                {
                    "row_index": idx,
                    "lesion_id": df.loc[idx, "lesion_id"],
    ...
    out_path = HAM10000_INTERIM_DIR / "value_validation_report.csv"
