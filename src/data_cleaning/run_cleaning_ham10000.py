"""Orchestrator: runs the full HAM10000 cleaning pipeline, steps 1-6.

data/raw/ is never written to (enforced by
common/io_utils.assert_not_raw_path, reused from the audit phase).
Intermediate outputs go to data/interim/HAM10000/, final outputs to
...
from src.data_cleaning.config import HAM10000_LOGS_DIR
from src.data_cleaning.ham10000 import (
...
def main() -> None:
    ...
    log_file = HAM10000_LOGS_DIR / f"cleaning_run_{timestamp}.log"
    logger = get_logger("ham10000_cleaning", log_file)

    logger.info("=" * 70)
    logger.info("HAM10000 DATA CLEANING - START")
    logger.info("=" * 70)
    ...
    try:
        ...
        logger.info("--- Step 4: Lesion-wise split ---")
    ...
    logger.info("=" * 70)
    logger.info("HAM10000 DATA CLEANING - COMPLETE")
    logger.info("=" * 70)
