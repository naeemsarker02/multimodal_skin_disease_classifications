...
Verifies the split actually did what it claims: zero patient overlap
between train/val/test, and reports per-split size and class balance
so any residual skew (e.g. from the 179 multi-diagnosis patients) is
visible and documented rather than assumed away.
"""

import pandas as pd

from src.data_audit.common.io_utils import save_csv
from src.data_cleaning.config import PAD_UFES20_PROCESSED_DIR


def run(logger, splits: dict) -> pd.DataFrame:
    patient_sets = {name: set(df["patient_id"]) for name, df in splits.items()}

    overlaps = []
    names = list(patient_sets.keys())
    for i in range(len(names)):
        for j in range(i + 1, len(names)):
            shared = patient_sets[names[i]] & patient_sets[names[j]]
            if shared:
                overlaps.append((names[i], names[j], len(shared)))

    if overlaps:
        logger.error("Patient leakage detected across splits: %s", overlaps)
        raise ValueError(f"Patient leakage detected: {overlaps}")
    logger.info("No patient overlap between train/val/test - split is leakage-free")
    ...
    out_path = PAD_UFES20_PROCESSED_DIR / "split_quality_report.csv"
    save_csv(report_df, out_path)

    for split_name, split_df in splits.items():
        logger.info(
            "%s: %d images (%.1f%% of total), %d patients",
            split_name,
            len(split_df),
