"""Orchestrator: runs the full PAD-UFES-20 dataset audit, modules 1-11
in order. Each module writes only to reports/PAD_UFES20/ and
logs/PAD_UFES20/ - data/raw/ is never written to (enforced by
...
# Allow running this file directly (not just via -m) by ensuring the
# project root is on sys.path.
PROJECT_ROOT = Path(__file__).resolve().parents[2]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from src.data_audit.config import PAD_UFES20_LOGS_DIR
from src.data_audit.common.logging_utils import get_logger
from src.data_audit.pad_ufes20 import (
...
def main() -> None:
    ...
    log_file = PAD_UFES20_LOGS_DIR / f"audit_run_{timestamp}.log"
    logger = get_logger("pad_ufes20_audit", log_file)

    logger.info("=" * 70)
    logger.info("PAD-UFES-20 DATASET AUDIT - START")
    logger.info("=" * 70)
    ...
    try:
        ...
        logger.info("--- Module 11: Patient statistics ---")
        results["patient_stats"] = m10_patient_statistics.run(logger)
    ...
    logger.info("=" * 70)
    logger.info("PAD-UFES-20 DATASET AUDIT - COMPLETE")
    logger.info("=" * 70)
