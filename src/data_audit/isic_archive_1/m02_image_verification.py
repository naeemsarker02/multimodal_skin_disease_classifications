...
file with Pillow, capturing dimensions/mode in the same pass (as in the
HAM10000 and PAD-UFES-20 audits, to avoid a second reopen-pass).
"""
...
from src.data_audit.config import ISIC1_FIGURES_DIR, ISIC1_RAW_DIR, ISIC1_REPORTS_DIR
from src.data_audit.common.io_utils import ensure_dir, save_csv


def _verify_one(rel_path: str) -> dict:
    full_path = ISIC1_RAW_DIR / rel_path
...
def run(logger, inventory_df: pd.DataFrame) -> pd.DataFrame:
    ...
    save_csv(df, ISIC1_REPORTS_DIR / "03_image_verification.csv")
    corrupted = df[df["status"] == "CORRUPTED"]
    save_csv(corrupted, ISIC1_REPORTS_DIR / "03_corrupted_images.csv")
    ...
    save_csv(stats_df, ISIC1_REPORTS_DIR / "04_image_size_stats.csv")
    save_csv(resolution_freq, ISIC1_REPORTS_DIR / "04_resolution_frequency.csv")
    ...
    fig.tight_layout()
    fig_path = ISIC1_FIGURES_DIR / "image_size_distribution.png"
    ...
    logger.info("Saved -> %s", ISIC1_REPORTS_DIR / "03_image_verification.csv")
    logger.info("Saved -> %s", ISIC1_REPORTS_DIR / "04_image_size_stats.csv")
