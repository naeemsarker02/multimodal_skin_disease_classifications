...
file with Pillow. This single read-only pass produces both the full
verification report (status + dimensions for every image) and the
corrupted-images report (the subset that failed to decode). Combined
for the same reason as in the PAD-UFES-20 audit: detecting corruption
*is* attempting verification, and dimensions are effectively free to
capture once the file is already open, so a second reopen-pass (for
image size stats) is avoided.
"""
...
def _verify_one(rel_path: str) -> dict:
    full_path = HAM10000_RAW_DIR / rel_path
    result = {
...
def run(logger, inventory_df: pd.DataFrame) -> pd.DataFrame:
    ...
    out_path = HAM10000_REPORTS_DIR / "03_image_verification.csv"
    save_csv(df, out_path)

    corrupted = df[df["status"] == "CORRUPTED"]
    corrupted_path = HAM10000_REPORTS_DIR / "03_corrupted_images.csv"
