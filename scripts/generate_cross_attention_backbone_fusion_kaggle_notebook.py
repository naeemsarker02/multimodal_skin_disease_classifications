"""Generates notebooks/pad_ufes20_cross_attention_backbone_fusion_kaggle_notebook.md.

Reads the actual Step 4 source files from src/models/ and embeds their
real content into %%writefile cells - never hand-typed, so the notebook
...
model/GPU check -> training cells), extended with a 4th "Add Data"
source and a setup-time copy step that Phase 7 Stage 2's notebook didn't
need.

Step 4 requires FOUR Kaggle "Add Data" sources, not three - the extra
one is new:
  ...
  4. NEW - a private dataset holding the Phase 8B/Step 3
     PAD_UFES20_Expanded backbone checkpoints (image_convnext_tiny_seed{0,1,2}_best.pt,
     image_densenet121_seed{0,1,2}_best.pt, 6 files from this machine's
     logs/PAD_UFES20_Expanded/checkpoints/) - image embedder warm start.
     naeemsarkertracer/pad-ufes20-expanded-backbone-checkpoints
     (https://www.kaggle.com/datasets/naeemsarkertracer/pad-ufes20-expanded-backbone-checkpoints).

Source (4) is not resolved via config.py's Kaggle-slug indirection
(PAD_UFES20_Expanded has no such entry - its checkpoints_dir is a plain
OUTPUT_ROOT-relative path, /kaggle/working/logs/PAD_UFES20_Expanded/checkpoints
on Kaggle) - so the setup cell below copies the 6 files there directly
before training starts, rather than extending config.py's Kaggle-slug
machinery for a single one-off notebook.

Run: python scripts/generate_cross_attention_backbone_fusion_kaggle_notebook.py
"""
...
OUT_PATH = PROJECT_ROOT / "notebooks" / "pad_ufes20_cross_attention_backbone_fusion_kaggle_notebook.md"

# Private Kaggle "Add Data" source holding
# logs/PAD_UFES20_Expanded/checkpoints/image_{convnext_tiny,densenet121}_seed{0,1,2}_best.pt
# (6 files only - not all 15 from Step 3).
...
FILES = [
    ...
    ("cross_attention_backbone_fusion_model.py", "/kaggle/working/src/models/cross_attention_backbone_fusion_model.py"),
    ("train.py", "/kaggle/working/src/models/train.py"),
    ("train_cross_attention_backbone_fusion.py", "/kaggle/working/src/models/train_cross_attention_backbone_fusion.py"),
]
...
def build_notebook() -> str:
    ...
    parts.append(
        ...
        "Trains CrossAttentionBackboneFusionModel for the two Phase 8B "
        "top-2 backbones (ConvNeXt-Tiny, DenseNet121) x 3 seeds = 6 runs, "
        "all on PAD_UFES20 (never PAD_UFES20_Expanded - "
        "image_branch_only=True blocks metadata/fusion training there; see "
        "Project_Tracking.md's Step 2 Integration Plan). Warm-starts are "
        "deliberately cross-dataset: the image embedder loads a Step 3 "
        "(Phase 8B) PAD_UFES20_Expanded backbone checkpoint, the metadata "
        "embedder loads the existing Phase 7 Stage 1 PAD_UFES20 metadata "
        "checkpoint - see train_cross_attention_backbone_fusion.py's "
        "docstring for the full reasoning.\n\n"
        "Requires **four** Kaggle \"Add Data\" sources attached before "
        "running (one more than Phase 7 Stage 2's notebook):\n\n"
        ...
        f"- `{EXPANDED_BACKBONE_CHECKPOINTS_SLUG}` (a private dataset "
        "containing this machine's "
        "`logs/PAD_UFES20_Expanded/checkpoints/image_convnext_tiny_seed{0,1,2}_best.pt` "
        "and `image_densenet121_seed{0,1,2}_best.pt` - 6 files total, the "
        "Step 3/Phase 8B backbone checkpoints. Attach this as its own "
        "\"Add Data\" source before running this notebook; there is no "
        "other way for this Kaggle session to see them, since "
        "`/kaggle/working` from the backbone-comparison notebook that "
        "produced them was already wiped.)\n\n"
    ...
    parts.append(
        f"## Cell {cell_num} — Full model/GPU/dependency check (cross-attention backbone model, warm-start, one real batch)\n\n"
        "```python\n"
        "import torch, sys\n"
    ...
    total_runs = len(BACKBONES) * len(SEEDS)
    run_idx = 0
    for backbone in BACKBONES:
        for seed in SEEDS:
            run_idx += 1
            parts.append(
                f"## Cell {cell_num} — Train: cross-attention backbone fusion, "
                f"backbone={backbone}, seed {seed}\n\n"
                "```python\n"
                f"!python -m src.models.train_cross_attention_backbone_fusion "
                f"--backbone {backbone} --seed {seed}\n"
                "```\n\n"
            )
