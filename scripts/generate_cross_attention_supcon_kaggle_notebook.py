"""Generates notebooks/pad_ufes20_cross_attention_supcon_kaggle_notebook.md.

Reads the actual source files from src/models/ and embeds their real
content into %%writefile cells - the notebook is never hand-typed, so it
cannot silently drift from the source-of-truth .py files. Mirrors
generate_cross_attention_kaggle_notebook.py (the headline Phase 7 Stage 2
notebook) exactly - same "Add Data" sources, same folder-verification /
sanity-check / full-model-check cell pattern - since this is the same
architecture and data, warm-started from the identical Stage 1
checkpoints; only the training entrypoint (train_cross_attention_supcon.py,
adds losses.py) and the loss function differ.

Reason this exists (2026-09-06): the local machine has ~7.4GB RAM and the
3-seed CPU run was repeatedly OOM-killed by the OS/harness even one seed
at a time. This offloads training to Kaggle's free GPU, same as the Step 3
5-backbone comparison did for the same reason (see
generate_backbone_comparison_kaggle_notebook.py).

Run: python scripts/generate_cross_attention_supcon_kaggle_notebook.py
"""
...
OUT_PATH = PROJECT_ROOT / "notebooks" / "pad_ufes20_cross_attention_supcon_kaggle_notebook.md"
...
FILES = [
    ...
    ("losses.py", "/kaggle/working/src/models/losses.py"),
    ("backbones.py", "/kaggle/working/src/models/backbones.py"),
    ("train.py", "/kaggle/working/src/models/train.py"),
    ("train_cross_attention_supcon.py", "/kaggle/working/src/models/train_cross_attention_supcon.py"),
]
...
def build_notebook() -> str:
    ...
    parts.append(
        "# PAD-UFES-20 Kaggle Notebook — Cross-Attention + Supervised Contrastive Loss (SupCon)\n\n"
        ...
        "**Why this notebook exists:** the local dev machine has ~7.4GB RAM and "
        "the 3-seed CPU run was repeatedly OOM-killed by the OS, even one seed "
        "at a time - this is not a code issue (a 1-epoch local smoke test "
        "passed cleanly first). Offloading to Kaggle's free GPU, same reason "
        "Step 3's 5-backbone comparison ran there.\n\n"
        "**This is a VAL-ONLY experiment - `train_cross_attention_supcon.py` "
        "never reads `metadata_test.csv`.** L = CE_loss (class-weighted, "
        "identical weights to the headline) + lambda * SupCon_loss (Khosla et "
        "al. 2020, lambda=0.1 default), applied to the 320-d fused "
        "(post-cross-attention, pre-classifier) feature vector. Same "
        "warm-start, LR, batch size, epoch cap, and early-stopping settings "
        "as the locked headline `train_cross_attention_fusion.py` run - only "
        "the loss function changes. Writes to new "
        "`cross_attention_supcon_*` checkpoint/log names; the locked headline "
        "checkpoints (`cross_attention_seed{0,1,2}_best.pt`) are never opened "
        "for writing.\n\n"
        "Structured for \"Save & Run All (Commit)\", same process as every "
        "prior notebook. Requires **three** Kaggle \"Add Data\" sources "
        "attached before running - identical sources to the headline "
        "cross-attention notebook, since this warm-starts from the same "
        "Stage 1 image/metadata checkpoints:\n\n"
        ...
        "`image_seed{0,1,2}_best.pt` / `metadata_seed{0,1,2}_best.pt`) - this "
        "run cannot proceed without them.\n\n"
        "After \"Save & Run All (Commit)\" finishes, download the notebook's "
        "Output (Kaggle preserves everything under `/kaggle/working`) and "
        "copy back into this repo:\n"
        "- `logs/PAD_UFES20/checkpoints/cross_attention_supcon_seed{0,1,2}_best.pt`\n"
        "- `logs/PAD_UFES20/train_cross_attention_supcon_seed{0,1,2}.csv` (per-epoch metrics)\n"
        "- `logs/PAD_UFES20/train_cross_attention_supcon_seed{0,1,2}_summary.json`\n"
        "- `reports/PAD_UFES20/cross_attention_supcon/supcon_val_results.json` (combined, all 3 seeds)\n\n"
    ...
    parts.append(
        ...
        "    \"Stage 1 checkpoints not found in either expected layout - warm-start \"\n"
        "    \"cannot proceed. Re-check the uploaded dataset's contents/packaging.\"\n"
        ")\n\n"
    ...
    parts.append(
        f"## Cell {cell_num} — Full model/GPU/dependency check (cross-attention model, warm-start, SupCon loss, one real batch)\n\n"
        "```python\n"
        "import torch, sys\n"
        ...
        "assert logits.shape == (8, ds_config.num_classes)\n"
        "assert features.shape == (8, 320), f\"expected 320-d joint feature, got {features.shape}\"\n"
        "print(\"forward pass OK - logits:\", logits.shape, \"features:\", features.shape)\n\n"
        "supcon = SupConLoss(temperature=0.07)\n"
        "loss = supcon(features, labels)\n"
        "print(\"SupCon loss on one real batch:\", loss.item())\n\n"
    ...
    for seed in (0, 1, 2):
        parts.append(
            f"## Cell {cell_num} — Train: cross-attention + SupCon, seed {seed}\n\n"
            "```python\n"
            f"!python -m src.models.train_cross_attention_supcon --seed {seed} --lambda-supcon 0.1\n"
            "```\n\n"
        )
        parts.append("---\n\n")
        cell_num += 1
    ...
    parts.append(
        f"## Cell {cell_num} — Combine all 3 seeds' val results (mirrors `main()`'s combined-results write)\n\n"
        "```python\n"
        "import json\n"
        "from src.models.config import get_dataset\n\n"
        "ds_config = get_dataset(\"PAD_UFES20\")\n"
        "results = []\n"
        "for seed in (0, 1, 2):\n"
        "    p = ds_config.logs_dir / f\"train_cross_attention_supcon_seed{seed}_summary.json\"\n"
        "    with open(p) as f:\n"
        "        results.append(json.load(f))\n\n"
        "out_dir = ds_config.fusion_reports_dir.parent / \"cross_attention_supcon\"\n"
        "out_dir.mkdir(parents=True, exist_ok=True)\n"
        "with open(out_dir / \"supcon_val_results.json\", \"w\") as f:\n"
        "    json.dump(results, f, indent=2)\n\n"
        "macro_f1s = [r[\"val_macro_f1\"] for r in results]\n"
        "mean = sum(macro_f1s) / len(macro_f1s)\n"
        "std = (sum((x - mean) ** 2 for x in macro_f1s) / len(macro_f1s)) ** 0.5\n"
        "print(\"per-seed val macro-F1:\", macro_f1s)\n"
        "print(f\"mean +/- std: {mean:.4f} +/- {std:.4f}\")\n"
        "print(\"vs. headline baseline: 0.6209 +/- 0.0143\")\n"
        "for r in results:\n"
        "    print(f\"\\nseed {r['seed']} per-class F1:\")\n"
        "    for cls, f1 in r[\"val_per_class_f1\"].items():\n"
        "        print(f\"  {cls}: {f1:.4f}\")\n"
        "print(f\"\\ncombined results -> {out_dir / 'supcon_val_results.json'}\")\n"
        "```\n"
    )
